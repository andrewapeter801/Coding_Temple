...
```{r}
cod_reviews <- read_csv("Call of Duty Reviews.csv")
cod_reviews
```

```{r}
# Convert to a tibble
tibby_cod <- as_tibble(cod_reviews)
#view(tibby_cod)
```
...
rename_cols <- function(x){
  #Retrieve the column names using names(tibby_cod)
...
```{r}
my_new_tibby <- rename_cols((tibby_cod))
#view(my_new_tibby)
```

```{r}
# Check for missing values
sum(is.na(my_new_tibby))
```

```{r}
# Filter the tibble to remove rows with missing values

my_new_tibby <- my_new_tibby %>%
  drop_na()
...
#Mutate a column
cod <- my_new_tibby %>%
  mutate(my_new_tibby, avg_score = (critic_compound_score+user_compound_score)/2)
view(cod)
```
...
avg_user_rating <- mean(cod$user_compound_score)

ggplot(cod) +
  geom_histogram(binwidth = 0.05, aes(x=user_compound_score), fill = "blue", color="black")+ 
  goem_vline(xintercept =avg_user_rating, color='white', linetype = 'dashed') +
...
The t-test suggest that there is some evidence against the null hypothesis (p-value = 0.05508). The alternative hypothesis suggests that the trye difference in means is not equal to 0, indicating that there may be a significant differnce between user_compound_score and critic_compound_score. HOWEVER, the evidence is not exrtremely strong, as the p-value is slightly above the significance level of 0.05.
