...
```{r}
# comments are written with a pound sign - no doc strings!
```
...
```{r}
str1 <- "Logan scored 22/25 on his exam"
sprintf("%s scored a %d percent on his exam.", 'Logan', round((22/25)*100,2))
```

```{r}
str2 <- "Andrew is 40 years old"
str_2 <- sprintf("%d years old", 40)
sub('40', '41', str_2)
```
...
student_grade <- c(94,96,42,98,75,85)
student_names <- c('James','Kali','Lee','Whitney','Alex', 'Katie')
student_grade_level <- c('Freshman', 'Sophmore', 'Junior','Freshman','Senior','Freshman')
...
```{r}
student_grade <- df$student_grade
student_grade
```
...
```{r}
#Get first 2 rows in all the columns
df[1:2,]
```

```{r}
#Get first and 3rd rows
df[c(1,3),]
```
...
student_grade_level <- c('Junior','Sophmeore','Sophmore')
